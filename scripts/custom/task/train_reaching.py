"""
Task Learning training – combined locomotion + arm reaching task.
Same PPO setup as train.py but uses G1HybridGymEnvReaching (task_goal_dim=8).
"""
...
def main():
    parser = argparse.ArgumentParser("Task Learning (Reaching) — rl_games PPO MultiDiscrete")
    ...
    parser.add_argument(
        "--log_dir", type=str,
        default=f"./logs/task_reaching/{datetime.datetime.now().strftime('%d_%m_%Y_%H%M%S')}",
    )
    parser.add_argument("--run_name", type=str, default="g1_arm_reaching")
    ...
    base_env = G1HybridGymEnvReaching(cfg=env_cfg, render_mode=None)

    # Read dims from env — task_goal_dim=8 (2 vel + 2 arms * 3D)
    task_goal_dim = base_env.task_goal_dim
    s_dim = base_env.s_dim
    ...
    if cfg_path.exists():
        ...
        raise FileNotFoundError(f"[REACHING TRAIN] Config not found: {cfg_path}")
    ...
    rl_config["params"]["config"]["score_to_win"] = float(
        cfg["task_learning_policy"]["rl_config"]["params"]["config"]["score_to_win"]
    )

    from isaaclab_rl.rl_games import RlGamesVecEnvWrapper
    env_rl = RlGamesVecEnvWrapper(
        base_env,
        rl_device=device,
        clip_obs=rl_config["params"]["env"]["clip_observations"],
        clip_actions=rl_config["params"]["env"]["clip_actions"],
    )

    obs_reset = base_env.reset()
    if isinstance(obs_reset, tuple):
        obs_reset = obs_reset[0]
    full_obs_dim = obs_reset["policy"].shape[-1]
    print(f"[INFO] obs_dim={full_obs_dim}, s_dim={s_dim}, task_goal_dim={task_goal_dim}")
    assert full_obs_dim == s_dim + task_goal_dim, (
        f"obs_dim mismatch: {full_obs_dim} != {s_dim} + {task_goal_dim}"
    )
